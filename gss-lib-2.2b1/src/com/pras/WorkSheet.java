...
public class WorkSheet {
	 ...
	 */
	private Entry entry;
	private String worksheetStoragePath = "last-set-worksheet.txt";
	...
	public WorkSheet(String worksheetStoragePath){
		this.worksheetStoragePath = worksheetStoragePath;
	}
	...
	public ArrayList<WorkSheetRow> getData(boolean isCachedData){
		return getData(isCachedData, false, null, null, null);
	}
	
	public ArrayList<WorkSheetRow> getData(boolean isCachedData, String timePeriodQuery){;
		
		return getData(isCachedData, false, timePeriodQuery, null, null);
		
	}
	
//	public ArrayList<WorkSheetRow> getData(boolean isCachedData, Context context){
//		return getData(isCachedData, false, null, null, context);
//	}
	 ...
	 * @return
	 */
	public ArrayList<WorkSheetRow> getData(boolean isCachedData, boolean doReverse, String sq, String orderBy, Context context){
		...
		if(isCachedData){
			...
			// first load, from a file
			else{
				
				// don't know where to load it from, return empty
				if (context == null){
					return new ArrayList<WorkSheetRow>();
				}
				
				return loadRecordsFromFile(worksheetStoragePath, context);
			}
		}

		// explicit sync with the server
		...
		// save if the context was given
		if (context != null){
			saveToFile(context, worksheetStoragePath, xmlOut);
		}
		
		records = extractRecordsFromXml(xmlOut);
		return records;
	}
	
	
	private ArrayList<WorkSheetRow> loadRecordsFromFile(String worksheetStoragePath, Context context) {
		...
		try{
			...
			InputStream inputStream = context.openFileInput(worksheetStoragePath);
			...
			return extractRecordsFromXml(sb.toString());
			
		}
		catch (Exception e) {
			Log.p(TAG, "Exception occured while storing the spreadsheet");
		}
		return new ArrayList<WorkSheetRow>();
	}

	private ArrayList<WorkSheetRow> extractRecordsFromXml(String xmlOut) {
		// XML Parsing
	...
	private void saveToFile(Context context, String path, String xmlOut) {
